	...
	public virtual void Awake ()
	{
		...
		foreach (T searched in check)
		{
			if (searched!=this)
			{
				...
				{
                    if (GetComponent<BackgroundMusic>().SoundClip == null || searched.GetComponent<BackgroundMusic>().SoundClip == null)
                    {
                        Destroy(searched.gameObject);
                    }
                    else
                    {
                        if (searched.GetComponent<BackgroundMusic>().SoundClip.name == this.gameObject.GetComponent<T>().GetComponent<BackgroundMusic>().SoundClip.name)
                        {
                            Destroy(gameObject);
                        }
                        else
